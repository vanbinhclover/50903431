...
this.bodyStyle = 'background-color: #FFF6E0; font-family: "Verdana"; font-size: x-small;';
...
//    ['fontstyle'],
  ...
  ['strikethrough','subscript','superscript','separator'],
    ...
    ['justifyleft','justifycenter','justifyright','justify','separator'],

    ['OrderedList','UnOrderedList','Outdent','Indent','separator'],

    ['forecolor','backcolor','separator'],

    ['HorizontalRule','Createlink','InsertImage','InsertTable','custom2','separator'],
...
this.btnList = {
    ...
    "justify":   			['JustifyFull',       	 'Justify',      			 'editor_action(this.id)',  'icon_block.gif'],
    ...
    "custom2":           ['custom2',         'Incert Character',  'editor_action(this.id)',  'incert_char.gif'],
...
function editor_event(objname,runDelay) {
  ...
  // catch keypress events
    ...
    if (editEvent && editEvent.keyCode) {
	  ...
	  if (ctrlKey && (ord == 105 || ord == 73)) {     // catch ctrl-i (Insert Image)

		editEvent.returnValue = false;
		editor_obj.contentWindow.event.keyCode = 16;
		editor_action('_' +objname+ '_InsertImage');
		
        return;

      }

	  if (ctrlKey && (ord == 106 || ord == 74)) {     // catch ctrl-J (JustifyFull)

		editEvent.returnValue = false;
		editor_obj.contentWindow.event.keyCode = 16;
		editor_action('_' +objname+ '_JustifyFull');
	  ...
	  if (ctrlKey && altKey && (ord == 99 || ord == 67)) {     // catch ctrl+Alt-C (Fore Color)

		editEvent.returnValue = false;
		editor_obj.contentWindow.event.keyCode = 16;
		editor_action('_' +objname+ '_ForeColor');
		
        return;

      }
	  
	  if (ctrlKey && altKey && (ord == 114 || ord == 82)) {     // catch ctrl+Alt-R (View Source)

		editEvent.returnValue = false;
		editor_obj.contentWindow.event.keyCode = 16;
		editor_setmode(objname,'init');
//		editor_action('_' +objname+ '_HtmlMode');
...
function editor_updateToolbar(objname,action) {
  ...
  var IDList = Array('Bold','Italic','Underline','StrikeThrough','SubScript','SuperScript','JustifyLeft','JustifyCenter','JustifyRight','Justify','InsertOrderedList','InsertUnorderedList');
