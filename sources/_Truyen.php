...
var $html=array();
var $cat="TruyenNgan";
var $name="Truy&#7879;n Ng&#7855;n";
// Start func
function sMain(){
global $DB,$func,$NDK,$input;

	$this->cat=$input['sub'];
	$qr=$DB->query("SELECT cname FROM category WHERE pact='{$this->cat}'");
	if ($cat=$DB->fetch_row($qr)) {
		$this->name=$cat['cname'];
	} else {
	...
	if (empty($input['id'])) {
		...
		$data['nav']="<a href=\"?cmd=act:Truyen|sub:{$this->cat}\">{$this->name}</a> &raquo; ".$func->Get_Title($input['id']);
...
while ($row=$DB->fetch_row($query)) {
	$row['stt']=$start+$i;
	$row['ten']="<a href=\"?cmd=act:Truyen|sub:{$this->cat}|id:{$row['id']}\"><b>{$row['ten']}</b></a>";
    $list[]=$row;
	$i++;
}
$table['row'] = $list;
$textout.= $func->ShowAlpha("Truyen|sub:".$this->cat,$input['alpha']);
...
if (file_exists("images/title/".$input['sub'].".gif")) {
	$title="<img src=\"images/title/".$input['sub'].".gif\" border=0>";
}
