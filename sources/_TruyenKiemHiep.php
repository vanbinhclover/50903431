...
var $baseurl="";
var $html=array();
var $cat="TruyenKiemHiep";
var $name="Truy&#7879;n Ki&#7871;m Hi&#7879;p";
// Start func
function sMain(){
...
global $DB,$func,$NDK,$input;
	...
	$alpha=$input['alpha'];
	$p=intval($input['p']);
$where=" WHERE tid!=0 AND cat='TruyenKiemHiep'";
$order=" ten ASC ";
...
$textout.= $func->ShowListDai("TruyenKiemHiep");
...
$query = $DB->query("SELECT * FROM truyendai WHERE tid='{$id}' AND cat='TruyenKiemHiep'");
if ($truyen=$DB->fetch_row($query)) {
	...
	if ($tacgia=$DB->fetch_row($qr)) {
		...
		$k=0;
		$qr=$DB->query("SELECT * FROM truyendai WHERE tacgia='{$truyen['tacgia']}' AND cat='TruyenKiemHiep' ORDER BY ten ASC");
